...
(function ($) {
	...
	if ( settings ) {
		...
		var checkSettingsInterval = setInterval( checkSettingsType, 1000 );
		var counterInterval = 0;
		function checkSettingsType () {
			counterInterval++;
			settings = $( paymentPlansContainerId ).data( 'settings' );
			if ( settings ) {
	...
	function almaWidgetInject() {
		console.log(' almaWidgetInject() ');
